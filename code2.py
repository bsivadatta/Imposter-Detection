...
def get_legal_data(indices,path):
	legal_data = np.zeros((len(indices),25,46))
...
def get_imposters(indices,path):
	imposters = np.zeros((len(indices),25,46)) 
	for index in range(len(indices)):
		u = Users[indices[index]]
		filepath = os.path.join(path,u+'/')
		for id,f in enumerate(sorted(os.listdir(filepath))):
...
def train_test_split(legal_data,imposters,N,M):
	X_train = np.zeros((200,46))
	y_train = np.zeros((200,1))
	X_test = np.zeros((50,46))
...
class ANN(nn.Module):
	...
	def __init__(self):
		...
		self.fc1 = nn.Linear(46, 100)  # 6*6 from image dimension
...
def train_model(X_train, y_train, X_test, y_test):
	...
	for epoch in range(120):
		...
		for i, (images, labels) in enumerate(train_loader):
			images = images.view(-1, 46).requires_grad_()
			optimizer.zero_grad()
...
ipath = os.path.join(root,"TrainData2/")
...
alp2 = ['TH','HE','AN','IN','ER','ND','RE','ED','ES','OU','TO','HA','EN','EA','ST','NT','ON','AT','HI','AS']

for s in subjects:
	encode(s,fpath,ipath)

print("Generating new features if data is insufficient")
for s in subjects:
	newfeatures(s,ipath)
